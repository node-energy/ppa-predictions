...
class SimplePredictor(AbstractPredictor):
    def configure(self, historic_load_profile_slice: HistoricLoadProfile, state, predict_days=7):
    ...
    def _get_df(self):
        ...
        data = list(map(lambda t: [t.datetime, t.value], historic_load_profile_slice.timestamps))
        ...
        historic_df['is_holiday'] = historic_df.index.to_series().apply(lambda d: d in state_holidays)
        historic_df['is_weekday'] = np.where(historic_df['day_of_week'] < 5, True, False)
        historic_df['is_friday'] = np.where(historic_df['day_of_week'] == 4, True, False)
        historic_df['is_saturday'] = np.where(historic_df['day_of_week'] == 5, True, False)
        historic_df['is_sunday'] = np.where(historic_df['day_of_week'] == 6, True, False)
        historic_df['month'] = pd.to_datetime(historic_df.index).month
        historic_df['hour'] = historic_df.index.to_series().apply(lambda d: d.hour + d.minute / 60)
...
class RandomForestPredictor(SimplePredictor):
    ...
    def create_prediction(self):  # TODO refactor
        historic_df = self._get_df()
        x = historic_df[
            [
                "month",
